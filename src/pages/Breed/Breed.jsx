...
export const Breed = ({isImageOpen}) => {
    ...
    return (
        <section className={styles.main}>
            <Helmet>
                <title>Стандарт породы прайтер</title>
                <meta name="description" content="Полное описание стандарта породы прайтер"></meta>
            ...
            <div className={styles.container}>
                ...
                < p className={styles.text}>Прайтер, собака с очень покладистым, универсальным характером, легко управляемый и обучаемый, очень ориентирован на подчинение человеку. Прайтер никогда не пытается доминировать и не пытается быть лидером в семье. Контактен и дружелюбен с любым человеком, даже не знакомым ему. Особенно обожает детей, готов терпеть любые шалости с их стороны. Не смотря на то, что Прайтер нежная и заботливая нянька для ребенка, если того потребует ситуация, то отлично справится и с защитой всей своей семьи. Прайтер, удивительно способен ко всем видам дрессировке и по этому уже прославился как отличник по всем ее дисциплинам. А так же и на спортивных состязаниях, Прайтер почти всегда занимает призовые места. Такие способности Прайтера открывают для его владельцев огромные перспективы в любых направлениях и Прайтер готов справится с любыми сложностями, его это не пугает, это его характер.</p>
